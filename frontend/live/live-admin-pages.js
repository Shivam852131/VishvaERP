(function () {
  ...
  async function initSuperAdminDatabasePage() {
    const colIcons = { users: 'fa-users', colleges: 'fa-university', fees: 'fa-money-bill', feestructures: 'fa-layer-group', attendances: 'fa-calendar-check', leaves: 'fa-file-alt', courses: 'fa-book', subjects: 'fa-graduation-cap', exams: 'fa-clipboard-list', results: 'fa-poll', timetables: 'fa-calendar-alt', examtemplates: 'fa-file-contract', questionbanks: 'fa-database', notes: 'fa-sticky-note', assignments: 'fa-tasks', submissions: 'fa-upload', liveclasssessions: 'fa-video', notices: 'fa-bullhorn', events: 'fa-calendar', communications: 'fa-comments', messages: 'fa-envelope', notifications: 'fa-bell', transports: 'fa-bus', hostels: 'fa-building', rooms: 'fa-door-open', studentprofiles: 'fa-id-card', skillassessments: 'fa-brain', assessmentattempts: 'fa-pen-to-square', placementcompanies: 'fa-building', placementjobs: 'fa-briefcase', placementapplications: 'fa-file-signature', placementdrives: 'fa-car', alumni: 'fa-user-tie', alumnidonations: 'fa-hand-holding-dollar', alumnievents: 'fa-glass-cheers', campusresources: 'fa-map-location-dot', campussensors: 'fa-satellite-dish', sensorreadings: 'fa-wave-square', energylogs: 'fa-bolt', sustainabilitygoals: 'fa-leaf', grievances: 'fa-exclamation-triangle', feedbacks: 'fa-star', inventories: 'fa-boxes-stacked', mentorships: 'fa-people-arrows', mentorsessions: 'fa-comments', classroomlocations: 'fa-location-dot', locationconsents: 'fa-shield-halved', livepresences: 'fa-circle-dot', subscriptions: 'fa-credit-card', payments: 'fa-receipt', otps: 'fa-key', broadcast: 'fa-tower-broadcast', auditlogs: 'fa-history', platformsettings: 'fa-gear' };
    const colColors = { users: '#4F46E5', colleges: '#10B981', fees: '#F59E0B', feestructures: '#F97316', attendances: '#0284C7', leaves: '#8B5CF6', courses: '#059669', subjects: '#0891B2', exams: '#EF4444', results: '#D97706', timetables: '#6366F1', examtemplates: '#A855F7', questionbanks: '#7C3AED', notes: '#EAB308', assignments: '#10B981', submissions: '#06B6D4', liveclasssessions: '#EC4899', notices: '#F59E0B', events: '#8B5CF6', communications: '#64748B', messages: '#0EA5E9', notifications: '#F43F5E', transports: '#78716C', hostels: '#B45309', rooms: '#0D9488', subscriptions: '#4F46E5', payments: '#059669', auditlogs: '#64748B', studentprofiles: '#0284C7', skillassessments: '#7C3AED', placementjobs: '#059669', grievances: '#EF4444', feedbacks: '#F59E0B', inventories: '#78716C' };
    const deletableCollections = new Set(['fees', 'feestructures', 'attendances', 'leaves', 'courses', 'subjects', 'exams', 'results', 'timetables', 'examtemplates', 'questionbanks', 'notes', 'assignments', 'submissions', 'notices', 'events', 'transports', 'hostels', 'rooms', 'studentprofiles', 'skillassessments', 'assessmentattempts', 'placementcompanies', 'placementjobs', 'placementapplications', 'placementdrives', 'alumni', 'alumnidonations', 'alumnievents', 'campusresources', 'campussensors', 'sensorreadings', 'energylogs', 'sustainabilitygoals', 'grievances', 'feedbacks', 'inventories', 'mentorships', 'mentorsessions', 'broadcasts', 'payments', 'subscriptions', 'communications', 'messages', 'notifications', 'liveclasssessions', 'classroomlocations', 'locationconsents', 'livepresences']);

    let allCollections = [];
    let collectionFilter = 'all';
    let collectionSearchQuery = '';
    let collectionSortBy = 'count';
    ...
    function fallbackCopy(text, successMsg) {
      ...
      try {
        ...
        window.showToast?.('Unable to copy', 'error');
      }
      document.body.removeChild(ta);
    }

    function formatCell(value, key) {
      if (value === null || value === undefined) return '<span style="color:#94A3B8;font-style:italic">null</span>';
      if (key === '_id' || (typeof value === 'string' && value.length === 24 && /^[a-f\d]+$/i.test(value))) {
        const valStr = String(value);
        const short = valStr.length > 12 ? valStr.slice(0, 10) + '...' : valStr;
        return `
          <span class="id-pill" title="Click to copy ID" onclick="event.stopPropagation();window.copyToClipboard('${escapeHTML(valStr)}')">
      ...
      if (key === 'isActive' || key === 'status') {
        ...
        const isPositive = value === true || str === 'active' || str === 'success' || str === 'paid' || str === 'approved';
        const isNegative = value === false || str === 'inactive' || str === 'blocked' || str === 'failed' || str === 'rejected';
        const typeClass = isPositive ? 'success' : isNegative ? 'danger' : 'warning';
        return `<span class="status-pill ${typeClass}"><span class="health-dot ${isPositive ? 'healthy' : 'degraded'}" style="width:6px;height:6px"></span>${escapeHTML(String(value))}</span>`;
      }
      if (key === 'createdAt' || key === 'updatedAt' || key === 'lastLogin' || key === 'paidDate' || key === 'dueDate') {
        return value ? `<span style="font-size:11px;color:#475569">${formatDate(value, true)}</span>` : '<span style="color:#CBD5E1">—</span>';
      }
      if (key === 'email' && typeof value === 'string') {
        return `<span class="inline-flex items-center gap-1.5" title="${escapeHTML(value)}"><i class="fas fa-envelope text-slate-400 text-xs"></i><span class="font-medium text-slate-700">${escapeHTML(value)}</span></span>`;
      }
      if (Array.isArray(value)) return `<span class="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded bg-slate-100 text-slate-600 font-semibold"><i class="fas fa-layer-group text-slate-400"></i> ${value.length} items</span>`;
      if (typeof value === 'object') return `<code style="font-size:10.5px;color:#7C3AED;background:#F5F3FF;padding:1px 6px;border-radius:4px">{ ${Object.keys(value).length} keys }</code>`;
    ...
    function renderCollectionList() {
      ...
      let filtered = allCollections.filter((item) => {
        if (query && !item.name.toLowerCase().includes(query)) return false;
        if (collectionFilter === 'active' && item.count <= 0) return false;
        if (collectionFilter === 'empty' && item.count > 0) return false;
        return true;
      });

      if (collectionSortBy === 'name') {
        filtered.sort((a, b) => a.name.localeCompare(b.name));
      } else if (collectionSortBy === 'size') {
        filtered.sort((a, b) => parseFloat(b.sizeMB || 0) - parseFloat(a.sizeMB || 0));
      } else {
        filtered.sort((a, b) => b.count - a.count);
      }

      setText('colCountBadge', String(filtered.length));

      if (!filtered.length) {
        setHTML('collectionList', `
          <div style="text-align:center;padding:36px 12px;color:#94A3B8">
            <i class="fas fa-folder-open" style="font-size:32px;opacity:0.4;margin-bottom:8px"></i>
            <div style="font-size:13px;font-weight:600;color:#64748B">No matching collections</div>
            <button class="btn btn-xs btn-secondary" style="margin-top:10px" onclick="clearColSearch()">Clear Filter</button>
          </div>
        `);
        return;
      }

      setHTML('collectionList', filtered.map((item) => `
        <div class="collection-card ${currentCollection === item.name ? 'active' : ''}" id="card-${item.name}" onclick="loadCollection('${item.name}', 1)">
          <div class="col-icon" style="${currentCollection === item.name ? '' : `background:${(colColors[item.name] || '#4F46E5')}14;color:${colColors[item.name] || '#4F46E5'}`}"><i class="fas ${colIcons[item.name] || 'fa-table'}"></i></div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;font-size:12.5px;color:#1E293B;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHTML(item.name)}</div>
            <div style="font-size:10.5px;color:#64748B;margin-top:1px">${item.sizeMB} MB • ${item.indexes} idx</div>
          </div>
          <div style="font-size:11px;font-weight:800;padding:2px 8px;border-radius:20px;background:${item.count > 0 ? '#EEF2FF' : '#F1F5F9'};color:${item.count > 0 ? '#4F46E5' : '#94A3B8'}">${item.count}</div>
        </div>
      `).join(''));
    }
    ...
    function renderDocs(docs, collection) {
      ...
      if (!docs.length) {
        const emptyHTML = `
          <div style="min-height:360px;display:flex;align-items:center;justify-content:center;color:#94A3B8;flex-direction:column;gap:10px;padding:40px 20px;text-align:center">
            <div style="width:56px;height:56px;border-radius:16px;background:#F1F5F9;display:flex;align-items:center;justify-content:center;font-size:24px;color:#94A3B8">
              <i class="fas fa-inbox"></i>
            </div>
            <div style="font-size:15px;font-weight:800;color:#334155">No documents found</div>
            <div style="font-size:12.5px;color:#64748B;max-width:320px">This collection currently has no records or no documents match your active search filter.</div>
          </div>
        `;
        setHTML('tableArea', emptyHTML);
        setHTML('cardsView', emptyHTML);
        return;
      }
        ...
        <table class="db-table erp-table">
          ...
          <tbody>
            ${docs.map((doc) => {
              ...
              return `
                <tr class="${isSelected ? 'selected' : ''}" onclick="window.inspectDoc('${docId}')" style="cursor:pointer">
                  <td style="text-align:center" onclick="event.stopPropagation()">
                    <input type="checkbox" class="doc-row-checkbox" ${isSelected ? 'checked' : ''} onchange="window.toggleDocSelection('${docId}', this.checked)" style="cursor:pointer">
                  </td>
                  ${displayKeys.map((key) => `<td class="doc-row">${formatCell(doc[key], key, doc)}</td>`).join('')}
                  <td style="text-align:right" onclick="event.stopPropagation()">
                    <div style="display:flex;gap:4px;justify-content:flex-end">
                      <button class="action-icon-btn btn-inspect" onclick="window.inspectDoc('${docId}')" title="Inspect Document"><i class="fas fa-eye"></i></button>
                      <button class="action-icon-btn" onclick="window.viewDocJSON('${encodeURIComponent(JSON.stringify(doc))}')" title="View Raw JSON"><i class="fas fa-code"></i></button>
                      <button class="action-icon-btn" onclick="window.copyDocJSON('${docId}')" title="Copy Document JSON"><i class="fas fa-copy"></i></button>
                      ${deletableCollections.has(collection) ? `<button class="action-icon-btn btn-danger" onclick="window.deleteDoc('${collection}','${docId}')" title="Delete Document"><i class="fas fa-trash"></i></button>` : ''}
                    </div>
                  </td>
                </tr>
              `;
      ...
      // Render Cards View
      setHTML('cardsView', docs.map((doc) => {
        const docId = String(doc._id || '');
        const isSelected = selectedDocIds.has(docId);
        const previewEntries = Object.entries(doc).filter(([k]) => k !== '_id' && k !== '__v').slice(0, 4);

        return `
          <div class="doc-preview-card ${isSelected ? 'border-indigo-500 bg-indigo-50/20' : ''}" onclick="window.inspectDoc('${docId}')" style="cursor:pointer">
            <div class="flex items-center justify-between pb-2 mb-2 border-b border-slate-100" onclick="event.stopPropagation()">
              <span class="id-pill" onclick="window.copyToClipboard('${escapeHTML(docId)}')">
                <code>${escapeHTML(docId.slice(0, 10))}...</code>
                <i class="fas fa-copy id-copy-btn"></i>
              </span>
              <input type="checkbox" ${isSelected ? 'checked' : ''} onchange="window.toggleDocSelection('${docId}', this.checked)" style="cursor:pointer">
            </div>
            <div class="flex flex-col gap-1.5 py-1">
              ${previewEntries.map(([k, v]) => `
                <div class="flex items-baseline justify-between gap-2 text-xs">
                  <span class="font-bold text-slate-500 font-mono text-[11px]">${escapeHTML(k)}:</span>
                  <span class="text-slate-800 text-right truncate font-mono text-[11px]">${escapeHTML(typeof v === 'object' ? JSON.stringify(v) : String(v))}</span>
                </div>
              `).join('')}
            </div>
            <div class="flex items-center justify-between pt-2 mt-2 border-t border-slate-100" onclick="event.stopPropagation()">
              <span class="text-[10px] font-semibold text-slate-400">${doc.createdAt ? formatDate(doc.createdAt, true) : ''}</span>
              <div class="flex items-center gap-1.5">
                <button class="action-icon-btn btn-inspect" onclick="window.inspectDoc('${docId}')" title="Inspect Record"><i class="fas fa-eye"></i></button>
                <button class="action-icon-btn" onclick="window.copyDocJSON('${docId}')" title="Copy JSON"><i class="fas fa-copy"></i></button>
                ${deletableCollections.has(collection) ? `<button class="action-icon-btn btn-danger" onclick="window.deleteDoc('${collection}','${docId}')" title="Delete Record"><i class="fas fa-trash"></i></button>` : ''}
              </div>
            </div>
          </div>
        `;
    ...
    async function loadCollection(name, page) {
      ...
      setHTML('browserTitle', `
        <i class="fas ${colIcons[name] || 'fa-table'}" style="color:${colColors[name] || '#4F46E5'}"></i>
        <span>${escapeHTML(name)}</span>
      `);

      const loadingHTML = `
        <div style="text-align:center;padding:50px 20px;color:#64748B">
          <i class="fas fa-circle-notch fa-spin" style="font-size:32px;color:#4F46E5;margin-bottom:12px"></i>
          <div style="font-size:13.5px;font-weight:700;color:#1E293B">Loading collection documents...</div>
          <div style="font-size:11.5px;color:#94A3B8;margin-top:4px">Fetching data partition from cluster</div>
        </div>
      `;
      setHTML('tableArea', loadingHTML);
      setHTML('cardsView', loadingHTML);

      try {
        const search = byId('docSearch')?.value || '';
        const res = await window.api.request(`/super-admin/database/collection/${name}?page=${currentPage}&limit=${currentLimit}&search=${encodeURIComponent(search)}`, { silent: true });
        currentDocs = res.docs || [];
        totalPages = res.pages || 1;
        byId('docCount').style.display = 'inline-flex';
        ...
        renderPagePills(res.page || 1, res.pages || 1);
        ...
        const errHTML = '<div class="empty-state"><div class="empty-state-title">Unable to load collection data</div></div>';
        setHTML('tableArea', errHTML);
        setHTML('cardsView', errHTML);
      }
    }

    function renderPagePills(current, total) {
      const container = byId('pagePills');
      if (!container) return;
      if (total <= 1) {
        container.innerHTML = '';
        return;
      ...
      for (let i = start; i <= end; i++) {
        html += `
          <button class="btn btn-xs ${i === current ? 'btn-primary' : 'btn-secondary'}" style="min-width:28px;font-weight:700" onclick="window.changePage(${i - current})">${i}</button>
        `;
      }
    ...
    async function loadStats() {
      try {
        ...
        setText('dbName', res.database?.name || 'test');
        ...
        setHTML('dbStatus', '<span class="health-dot healthy"></span><span>Connected</span>');

        allCollections = res.collectionStats || [];
        renderCollectionList();
      } catch {
        setHTML('dbStatus', '<span class="health-dot degraded"></span><span>Disconnected</span>');
    ...
    function switchView(mode) {
      ...
      if (byId('viewMode')) byId('viewMode').value = mode;

      const btnTable = byId('btnViewTable');
      const btnCards = byId('btnViewCards');
      const btnJSON = byId('btnViewJSON');
      if (btnTable) btnTable.classList.toggle('active', mode === 'table');
      if (btnCards) btnCards.classList.toggle('active', mode === 'cards');
      if (btnJSON) btnJSON.classList.toggle('active', mode === 'json');

      const tableView = byId('tableView');
      const cardsView = byId('cardsView');
      const jsonView = byId('jsonView');

      if (tableView) tableView.style.display = mode === 'table' ? '' : 'none';
      if (cardsView) cardsView.style.display = mode === 'cards' ? 'grid' : 'none';
      if (jsonView) jsonView.style.display = mode === 'json' ? '' : 'none';

      if (mode === 'json') {
        setText('jsonArea', JSON.stringify(currentDocs, null, 2));
      } else if (currentCollection) {
        renderDocs(currentDocs, currentCollection);
      }
    }

    // Inspector Drawer
    function inspectDoc(docId) {
      const doc = currentDocs.find((d) => String(d._id) === String(docId));
      if (!doc) return;
      activeInspectorDoc = doc;

      setText('drawerColName', currentCollection || 'Document');
      setText('drawerDocId', String(doc._id || '—'));
      setText('drawerTitle', doc.name || doc.title || doc.code || doc.email || 'Document Details');

      // Populate Fields Explorer
      const fieldsContainer = byId('drawerFieldsList');
      if (fieldsContainer) {
        fieldsContainer.innerHTML = Object.entries(doc).map(([k, v]) => {
          let typeStr = Array.isArray(v) ? 'array' : typeof v;
          if (v === null) typeStr = 'null';
          else if (k === '_id') typeStr = 'ObjectId';

          let displayVal = typeof v === 'object' && v !== null ? JSON.stringify(v, null, 2) : String(v);

          return `
            <div class="field-tree-row">
              <div class="field-key">
                <span>${escapeHTML(k)}</span>
                <span class="field-type-badge">${typeStr}</span>
              </div>
              <div class="field-val">
                <span>${escapeHTML(displayVal)}</span>
                <button class="text-slate-400 hover:text-indigo-600 ml-2" onclick="window.copyToClipboard('${escapeHTML(displayVal)}')" title="Copy Value"><i class="fas fa-copy text-xs"></i></button>
              </div>
            </div>
          `;
        }).join('');
      ...
      // Populate JSON code tab
      setText('drawerJsonCode', JSON.stringify(doc, null, 2));

      // Delete button visibility
      const delBtn = byId('drawerDeleteBtn');
      if (delBtn) {
        delBtn.style.display = deletableCollections.has(currentCollection) ? 'inline-flex' : 'none';
      }

      setDrawerTab('fields');
      byId('drawerOverlay')?.classList.add('active');
      byId('docInspectorDrawer')?.classList.add('active');
    }

    function closeInspector() {
      byId('drawerOverlay')?.classList.remove('active');
      byId('docInspectorDrawer')?.classList.remove('active');
      activeInspectorDoc = null;
    }

    function setDrawerTab(tab) {
      const tabFields = byId('tabFieldExplorer');
      const tabJson = byId('tabJsonCode');
      const drawerFieldTab = byId('drawerFieldTab');
      const drawerJsonTab = byId('drawerJsonTab');

      if (tab === 'fields') {
        tabFields?.classList.add('active');
        tabJson?.classList.remove('active');
        if (drawerFieldTab) drawerFieldTab.style.display = '';
        if (drawerJsonTab) drawerJsonTab.style.display = 'none';
      } else {
        tabFields?.classList.remove('active');
        tabJson?.classList.add('active');
        if (drawerFieldTab) drawerFieldTab.style.display = 'none';
        if (drawerJsonTab) drawerJsonTab.style.display = '';
      }
    }
    ...
    function toggleSelectAll(checked) {
      if (checked) {
        currentDocs.forEach((d) => selectedDocIds.add(String(d._id)));
      } else {
        selectedDocIds.clear();
      }
      qa('.doc-row-checkbox').forEach((cb) => { cb.checked = checked; });
      updateBulkActionBar();
    }
    ...
    function updateBulkActionBar() {
      ...
      const countEl = byId('selectedCount');
      if (!bar) return;
      if (selectedDocIds.size > 0) {
        if (countEl) countEl.textContent = String(selectedDocIds.size);
    ...
    function clearSelection() {
      selectedDocIds.clear();
      const selectAll = byId('selectAllCheckbox');
      if (selectAll) selectAll.checked = false;
      qa('.doc-row-checkbox').forEach((cb) => { cb.checked = false; });
      updateBulkActionBar();
    }
    ...
    window.deleteDoc = async function deleteDoc(collection, docId) {
      const confirmed = window.showConfirm
        ? await window.showConfirm({ title: 'Delete Document', message: `Permanently delete record ${docId} from ${collection}?`, confirmText: 'Delete', type: 'danger' })
    ...
    window.changeLimit = function changeLimit(val) {
      currentLimit = parseInt(val) || 20;
      if (currentCollection) loadCollection(currentCollection, 1);
    };
    ...
    window.exportCollection = function exportCollection() {
      if (!currentDocs.length) {
        window.showToast?.('Select a collection with documents first', 'warning');
        return;
      }
      const blob = new Blob([JSON.stringify(currentDocs, null, 2)], { type: 'application/json' });
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `${currentCollection || 'collection'}-page${currentPage}-${Date.now()}.json`;
      link.click();
      URL.revokeObjectURL(link.href);
      window.showToast?.(`Exported ${currentDocs.length} records as JSON`, 'success');
    };

    window.exportSelectedDocs = function exportSelectedDocs() {
      const selected = currentDocs.filter((d) => selectedDocIds.has(String(d._id)));
      if (!selected.length) return;
      const blob = new Blob([JSON.stringify(selected, null, 2)], { type: 'application/json' });
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `${currentCollection || 'collection'}-selected-${Date.now()}.json`;
      link.click();
      URL.revokeObjectURL(link.href);
      window.showToast?.(`Exported ${selected.length} selected records`, 'success');
    };

    window.bulkDeleteSelectedDocs = async function bulkDeleteSelectedDocs() {
      if (!selectedDocIds.size || !currentCollection) return;
      if (!deletableCollections.has(currentCollection)) {
        window.showToast?.('This collection is protected from bulk deletion', 'error');
        return;
      }
      const confirmed = window.showConfirm
        ? await window.showConfirm({ title: 'Bulk Delete', message: `Permanently delete ${selectedDocIds.size} records from ${currentCollection}?`, confirmText: 'Delete All', type: 'danger' })
        : window.confirm(`Permanently delete ${selectedDocIds.size} records from ${currentCollection}?`);
      if (!confirmed) return;

      try {
        await window.api.request(`/super-admin/database/collection/${currentCollection}/bulk-delete`, {
          method: 'POST',
          body: JSON.stringify({ docIds: Array.from(selectedDocIds) }),
        });
        window.showToast?.(`${selectedDocIds.size} documents deleted`, 'success');
        selectedDocIds.clear();
        updateBulkActionBar();
    ...
    window.refreshAll = async function refreshAll() {
      setHTML('collectionList', '<div class="skeleton skeleton-card" style="height:54px;border-radius:12px"></div>'.repeat(4));
      await loadStats();
      if (currentCollection) {
        await loadCollection(currentCollection, currentPage);
      }
      window.showToast?.('Cluster statistics refreshed', 'info');
    };

    // Sidebar Filtering & Sorting
    window.filterCollections = function filterCollections(val) {
      collectionSearchQuery = val || '';
      const clearBtn = byId('colSearchClear');
      if (clearBtn) clearBtn.style.display = collectionSearchQuery ? 'block' : 'none';
      renderCollectionList();
    };

    window.clearColSearch = function clearColSearch() {
      const input = byId('collectionSearch');
      if (input) input.value = '';
      window.filterCollections('');
    };

    window.setCollectionFilter = function setCollectionFilter(filter, btn) {
      collectionFilter = filter;
      qa('.col-filter-chip').forEach((c) => c.classList.remove('active'));
      if (btn) btn.classList.add('active');
      renderCollectionList();
    };

    window.sortCollections = function sortCollections(val) {
      collectionSortBy = val || 'count';
      renderCollectionList();
    };

    // Inspector Helpers
    window.inspectDoc = inspectDoc;
    window.closeInspector = closeInspector;
    window.setDrawerTab = setDrawerTab;
    window.copyDrawerId = function copyDrawerId() {
      if (activeInspectorDoc) copyToClipboard(activeInspectorDoc._id, 'Document ID copied');
    };
    window.copyDrawerJson = function copyDrawerJson() {
      if (activeInspectorDoc) copyToClipboard(JSON.stringify(activeInspectorDoc, null, 2), 'Document JSON copied');
    };
    window.copyDocJSON = function copyDocJSON(docId) {
      const doc = currentDocs.find((d) => String(d._id) === String(docId));
      if (doc) copyToClipboard(JSON.stringify(doc, null, 2), 'Document JSON copied');
    };
    window.copyJsonArea = function copyJsonArea() {
      const pre = byId('jsonArea');
      if (pre) copyToClipboard(pre.innerText, 'Collection JSON copied');
    };
    window.exportDrawerDoc = function exportDrawerDoc() {
      if (!activeInspectorDoc) return;
      const blob = new Blob([JSON.stringify(activeInspectorDoc, null, 2)], { type: 'application/json' });
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `${currentCollection}-${activeInspectorDoc._id}.json`;
      link.click();
      URL.revokeObjectURL(link.href);
      window.showToast?.('Document exported', 'success');
    };
    window.deleteDrawerDoc = async function deleteDrawerDoc() {
      if (!activeInspectorDoc || !currentCollection) return;
      const id = activeInspectorDoc._id;
      closeInspector();
      await window.deleteDoc(currentCollection, id);
    };
